...
import { Measurement } from "@/Config/models";
...
export default {
    ...
    data() {
        return {
            ...
            form: this.$inertia.form(
                {
                    _method: "PUT",
                    ...cloneDeep(Measurement)
                },
                {
                    bag: "measurement_form",
    ...
    methods: {
        fetch() {
            this.fetching = true;
            this.$axios
                .get(route("measurement.show", this.id).url())
                .then(({ data }) => {
                    assign(this.form, pick(data, keys(Measurement)));
                })
                .catch((e) => (this.dialog = false))
                .then(() => (this.fetching = false));
        },
        reset() {
            delete this.$page.errorBags["measurement_form"];
            assign(this.form, Measurement);
        },
        method() {
            let method = "post";
            let url = route("measurement.store");

            if (!this.creating) {
                method = "put";
                url = route("measurement.update", this.id);
            }
